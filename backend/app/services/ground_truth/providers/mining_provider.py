...
logger = logging.getLogger("firms_app.mining_provider")

BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
OFFICIAL_CATALOG_PATH = os.path.join(BASE_DIR, "data", "ground_truth_catalogs", "official", "mining", "isro_bhuvan_india_mining.json")

class MiningActivityGroundTruthProvider(BaseGroundTruthProvider):
    """
    Authoritative provider for ISRO Bhuvan / IBM (Indian Bureau of Mines) & USGS MRDS active mining activity dataset.
    """
    def __init__(self, catalog_path: str = None):
        super().__init__("ISRO_BHUVAN_IBM_MINING_OFFICIAL")
        if catalog_path:
    ...
    def _load_catalog(self) -> List[Dict[str, Any]]:
        if not os.path.exists(self.catalog_path):
            logger.warning(f"ISRO Bhuvan Mining catalog file not found at {self.catalog_path}")
            return []
        try:
            with open(self.catalog_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                records = data.get("records", data) if isinstance(data, dict) else data
                # Semantic filter: Explicitly ensure record category is MINING_ACTIVITY
                return [r for r in records if isinstance(r, dict) and r.get("source_category", "MINING_ACTIVITY") == "MINING_ACTIVITY"]
        except Exception as e:
            logger.error(f"Failed to load Mining Activity catalog: {e}")
            return []
        ...
        for rec in self._cached_records:
            ...
            evidence_list.append(
                GroundTruthEvidence(
                    ...
                    source_type="OPEN_PIT_MINING_LEASE_BOUNDARY",
                    source_record_id=rec.get("source_record_id") or rec.get("incident_id") or "INC_UNKNOWN",
                    class_label=GroundTruthClass.MINING_ACTIVITY,
                    latitude=rec_lat,
                    longitude=rec_lon,
                    event_start=t_start,
                    event_end=t_end,
                    ...
                    metadata={
                        "mine_name": rec.get("mine_name"),
                        "mineral_type": rec.get("mineral_type"),
                        "lease_area_ha": rec.get("lease_area_ha"),
                        "dataset_version": rec.get("dataset_version", "ISRO_BHUVAN_V2.0"),
                        "doi": rec.get("doi", "10.5067/MRDS/GLOBAL.001")
                    }
                )
            )
